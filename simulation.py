...
#    assert len(ds.tas.values) == 50*365, len(ds.tas.values)
    for i in range(5):
...
def detrend_time_series(ds, scenario):
    ...
    if scenario == "usual":
        ...
        assert np.round(np.mean(tas_hist.tas.values),2) == np.round(np.mean(tas_fut.tas.values),2), "remove mean trend failed"
    elif scenario == "rm_sd_trend":
        tas_fut = rm_sd_trend(tas_fut, mean_hist, sdev_hist)
        assert np.round(np.std(tas_hist.tas.values),2) == np.round(np.std(tas_fut.tas.values),2), "remove sdev trend failed"
...
#    assert adfuller(tas_hist.tas.values)[1] < 0.05, "Historical time series is not stationary"
...
def run_simulation(s):
    ...
    data_dir = "/scratch/duffy.k/climate_ts/"
    ...
    for i, file in enumerate(files):
        ...
        ds_out.to_netcdf("./pop_ts/"+file.split("/")[-1])
...
#AWI-CM-1-1-MR, EC-Earth3: Stomoxys.calcitrans
#'/scratch/duffy.k/climate_ts/EC-Earth3_Stomoxys.calcitrans.nc'
